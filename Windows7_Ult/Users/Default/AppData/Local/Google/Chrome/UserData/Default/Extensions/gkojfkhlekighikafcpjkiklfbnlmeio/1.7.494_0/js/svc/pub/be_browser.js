...
define(['jquery', '/util/etask.js', '/util/zerr.js', '/svc/pub/be_msg.js',
    ...
    version_util, be_transport, zua, zutil){
// XXX bahaa: rm
...
E.init = function(opt){
	...
	return;
    E.inited = true;
    E.browser_version = zua.guess_browser().version;
    E.os = zua.guess().os;
    opt = opt||{};
...
function firefox_have(){
    ...
    if (vcmp('1.3.630')>=0 && E.browser_version>=28)
    {
        ...
        if (E.os=='windows' && vcmp('1.4.536')>=0)
            have['be.plugin.add_fw_rule'] = true;
    }
    if (vcmp('1.3.920')>=0)
        have['tabs.disconnect'] = true;
    if (vcmp('1.5.733')>=0)
...
E.be = {
    ...
    plugin: {
        init: function(cb){
            E.rpc.call_api('be', 'plugin', 'init', [], cb); },
        start: function(cb){
        ...
        add_fw_rule: function(path, cb){
            E.rpc.call_api('be', 'plugin', 'add_fw_rule', [path], cb); },
	...
	del_listener: function(cb){ E.rpc.del_listener('be', 'plugin', cb); },
        ...
        enabled_state: function(mime, state, cb){
            E.rpc.call_api('be', 'plugin', 'enabled_state', [mime, state], cb);
        },
    },
